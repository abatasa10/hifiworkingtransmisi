...
var tokens = (v) => {
	return str(v).replace(/,/g, ";").split(";").map((s) => s.trim().toUpperCase()).filter(Boolean);
};
var riskNumbers = (v) => {
	const out = [];
...
function parseFlexibleWorkbook(wb, ctx, override) {
	...
	const mergePayload = (p) => {
		...
		for (const c of p.connections) {
			if (!connections.some((x) => x.from_external_key === c.from_external_key && x.to_external_key === c.to_external_key && x.relation_type === c.relation_type)) connections.push(c);
...
function parseEngineWorkbook(wb, filename) {
	...
	for (let i = 0; i < connections.length; i++) {
		...
		const k = `${c.from_external_key}\u0000${c.to_external_key}\u0000${c.relation_type}\u0000${c.relation_type === "IBT_LINK" ? c.unit_no ?? "1" : ""}`;
		if (seenConn.has(k)) {
...
//#endregion
//#region scripts/pipeline-test/main.ts
var sample = process.argv[2];
if (!sample) throw new Error("Pass an SLD engine workbook path: node scripts/pipeline-test/main.ts <workbook.xlsx>");
var { payload, issues } = parseWorkbookToPayload(XLSX.read(readFileSync(sample)), "ss_cwd_ingest.xlsx");
var tierMap = computeTiers(payload);
var vm = toViewModel(payload, tierMap, "Cawang");
